...
// const run10 = new Running([39, -12], 5.2, 24, 178);
// const cycling12 = new Cycling([39, -12], 27, 95, 523);
...
class App{
  ...
  _hideForm() {
    // clear input fields
    inputDistance.value = inputDuration.value = inputCadence.value = InputEvent.value = "";
  ...
  _newWorkout(e) {
    ...
    // if workout running, new running card
    if(type === "running") {
    ...
    // if workout cycling, new cycling card
    if(type === "cycling") {
      const elevation = +inputElevation.value;
    
      // check data
      if (!validInputs(distance, duration, elevation) || !allPositive(distance, duration)) 
      return alert("Please enter positive vlues.")
      
      workout = new Cycling([lat, lng], distance, duration, elevation);
    };

    // add card to wrokout list
  ...
  _renderWorkoutMarker(workout) {
    L.marker([ lat, lng ]).addtop(this.#map).bindPopup(L.popup({maxWidth: 250, minWidth: 100, autoClose: false, closeOnClick: false, className: `${workout.type}-popup`})).setPopupContent(`${workout.type === 'running' ? '🏃‍♂️' : '🚴‍♀️'}${workout.description}`).openPopu();
  };
